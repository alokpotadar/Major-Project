...
for segment in n:
    ...
    for midspan_incidence in midspan_incidence_arr:
        ...
        for i,theta in enumerate(theta_arr):
            l_arr=[]
            k=0
            c=midspan_chord + amplitude*0.5*((np.sin((y_s[i]*2*np.pi/lamda)+phase)));c_s.append(c);''';theta = np.arctan((c*np.tan(theta))/midspan_chord)'''
        ...
        C_l_spanwise=np.pi*aspect_ratio*np.array(x)
...
with open('Sine_phase_'+args.phase+'_A_'+str(amplitude/10)+'_w_'+args.wavelength+'.csv','w',newline='\n') as file:
