...
parser.add_argument("amplitude",type=str,nargs='+',choices=['3','5','7','9'])
...
for a in amplitude:
	filenames.append('Simulations_'+function+'/'+function+'_A_0.00'+a+'_w_'+wavelength+'.csv')
print(filenames)
C_l=[];C_d=[];Cl_upon_Cd=[]
...
with PdfPages(args.pdf_output_name) as pdf:
	...
	for i,filename in enumerate(filenames):
		plt.plot(x,C_l[i],linestyle='dashed',marker='.',label=function+' A'+amplitude[i]+r'$ \lambda$'+wavelength,alpha=0.8,linewidth=0.8)
	print('Cl :',C_l[-1][0],'-------',C_l[-1][-1])
	plt.xlabel('AoA (degrees)')
	plt.ylabel('$C_{L}$')
	# plt.title('Effect of Amplitude on $C_{l}$')
	plt.legend()
	...
	for i,filename in enumerate(filenames):
		plt.plot(x,C_d[i],linestyle='dashed',marker='.',label=function+' A'+amplitude[i]+r'$ \lambda$'+wavelength,alpha=0.8,linewidth=0.8)
	print('Cd :',C_d[-1][0],'-------',C_d[-1][-1])
	plt.xlabel('AoA (degrees)')
	plt.ylabel('$C_{Di}$')
	# plt.title('Effect of Amplitude on $C_{d}$')
	plt.legend()
	pdf.savefig(bbox_inches = 'tight')
	plt.close()
	plt.figure()
	...
	for i,filename in enumerate(filenames):
		plt.plot(x,Cl_upon_Cd[i],linestyle='dashed',marker='.',label=function+' A'+amplitude[i]+r'$ \lambda$'+wavelength,alpha=0.8,linewidth=0.8)
	print('Clcd :',Cl_upon_Cd[-1][0],'-------',Cl_upon_Cd[-1][-1])
	plt.xlabel('AoA (degrees)')
	plt.ylabel('$C_{L}/C_{Di}$')
	# plt.title('Effect of Amplitude on $C_{L}/C_{Di}$')
	plt.legend()
	pdf.savefig(bbox_inches = 'tight')
	plt.close()
